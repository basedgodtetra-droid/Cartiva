...
| P0 D27 Worker encryption string encoding | Deployed Sites OAuth start returned500 although Node tests and Vercel start passed. Worker cipher input requires explicit string encoding. | Shared encrypted envelopes pass UTF-8 explicitly, matching existing customer-token encryption. Unicode/AAD regression and real deployed OAuth start/cancel/replay checks cover the fix; unexpected start errors no longer expose internal exception text. |
...
`pnpm test:shared-state` runs34 shared persistence/transport/OAuth/cart contract tests against real SQLite statements and controlled retailer responses. It is also included in `test:discovery`. `pnpm test:shared-state:live` explicitly opts into a bounded real-D1 probe with a new synthetic owner; it never touches customer rows or retailer APIs. The deployed probe passed5 groups with28 bridge calls: one-of-four lease winner, one-use callback, atomic cart claim/owner isolation, disconnect/review fencing, and3-of8 shared quota. No real cart was written. Vercel's deployed start/cancel/replay checks passed. Initial Sites start exposed D27, fixed and retested as part of rollout.

Follow-up full suite before D27:115 files /2,139 tests passed. CARTIVA100 and all502 CARTIVA500 cases remain score2+ with no unsafe selections; representative live subset12/12 with15 requests (~30s). Updated browser suite passes11 groups locally and on deployed Vercel, including ownerless legacy recovery. Both production builds passed. Real five-item comparison passed on both hosts; two products honestly retain “Check availability.” These tests do not claim an actual shopper cart was modified.

### Shared-state deployment verification — September4,2026

- Final runtime source: `1d8ce98cb2532e69f6d62d6e1297f0895c36f634`; Sites version56 succeeded and Vercel production reached READY.
- Final full regression:115 files /2,140 tests, including34 shared-state cases. Scoped lint and both deployment builds passed.
- Both live hosts: web connection status200/configured; OAuth start200 with opaque owner cookie; exact same-host registered callback; verified cancellation400; repeated callback rejected400. Both official Kroger authorization pages returned200 sign-in content, without a redirect-registration mismatch. No login credentials were entered.
- Real deployed D1 probe:5 groups passed using28 signed bridge requests and0 retailer requests; one synthetic test owner created and left revoked, with its test operation permanently reviewed/fenced. No historical customer rows were inspected or migrated.
- Private bridge without Sites authorization returns401. Signed-request replay returns409. Sites remains owner-only:one allowed account,zero groups/visitors.
- Both live five-item comparisons pass with real retailer products and preserved quantities. “Check availability” remains visible for unknown stock, rather than becoming “No match.” No actual cart PUT was sent.
- Deployed Vercel browser suite passes11 groups, including mobile containment and legacy ownerless recovery. Secret scan checked79 client-build files with0 matches for the new shared-state secret or private Sites token.
- Real authenticated cart acceptance remains NOT TESTED. Both supported browser-control runtimes fail initialization with the same missing-kernel-assets error. Isolated QA profiles cannot use the shopper's Kroger session. A shopper must reconnect/complete consent before this final real-cart check can be completed.

The Worker-specific compatibility check follows the supported [Cloudflare Node crypto API](https://developers.cloudflare.com/workers/runtime-apis/nodejs/crypto/); the deployed failure and successful rerun, not documentation alone, are the evidence for D27.

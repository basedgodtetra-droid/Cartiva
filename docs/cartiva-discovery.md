...
- **P1: distributed cart idempotency and refresh coordination.** A process map or `/tmp` receipt cannot guarantee exactly-once submission across separate serverless instances. Concurrent cookie-token refresh can also race after rotation. Requires an authorized transactional shared store, atomic operation ownership and versioned sessions/disconnect tombstones. No such service is configured; silently claiming this solved would be false. Current pending journal and unknown-outcome safeguards are preserved.
- **P1 infrastructure: global abuse limits.** Trusted-IP partition fixes ordinary collisions, not distributed enforcement. Durable edge quotas still required for production scale; nontrusted fallback is intentionally coarse.
...
These gaps prevent a claim that every production P0/P1 and real-cart scenario is closed. The implemented discovery system and verified fixes can ship without a redesign; the shared-store decision and authenticated retailer acceptance remain explicit follow-up work.

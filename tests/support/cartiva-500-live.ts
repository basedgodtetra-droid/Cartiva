...
import { cartiva500LiveCases } from "@/tests/support/cartiva-500";

export type Cartiva500LiveStatus = "LIVE_PASSED" | "LIVE_FAILED" | "EXTERNAL_BLOCKED";

export interface Cartiva500LiveCaseResult {
  id: string;
  input: string;
  resolvedRequest?: string;
  status: Cartiva500LiveStatus;
  reason: string;
...
const LIVE_ORACLES: Record<string, LiveOracle> = {
  "C100-L1-004": {
    requiredGroups: [["banana", "bananas"]],
    forbidden: ["pudding", "bread", "chips", "candy", "snack"],
  },
  "C100-L1-019": {
    requiredGroups: [["paper"], ["towel", "towels"]],
    forbidden: ["toilet paper", "bath tissue", "holder"],
  },
  "C100-L1-002": {
...
export interface Cartiva500LiveReport {
  suiteId: "cartiva-500-kroger-live";
  status: Cartiva500LiveStatus;
  checkedAt: string;
  location?: {
    locationId: string;
  ...
  blocked: number;
  failed: number;
  cases: Cartiva500LiveCaseResult[];
  reason?: string;
}
...
export function cartiva500LiveOracleFailure(testCaseId: string, product: KrogerProduct) {
...
export function cartiva500LiveFulfillmentFailure(
  testCase: ReturnType<typeof cartiva500LiveCases>[number],
...
export function cartiva500LiveIndependentCandidateReady(
  testCase: ReturnType<typeof cartiva500LiveCases>[number],
  ...
  return product.cartEligible
    ...
    && !cartiva500LiveOracleFailure(testCase.id, product)
    && Boolean(fulfillment && !cartiva500LiveFulfillmentFailure(testCase, product, fulfillment));
}

export function cartiva500LiveHandoffBlockReason(result: KrogerMatchResult) {
...
export function cartiva500LiveCaseDisposition(options: {
  ...
  const handoffBlock = cartiva500LiveHandoffBlockReason(options.result);
  return handoffBlock
...
function resolveLiveRequest(testCase: ReturnType<typeof cartiva500LiveCases>[number]) {
...
function blockedReport(reason: string, retailerCalls = 0): Cartiva500LiveReport {
  return {
    suiteId: "cartiva-500-kroger-live",
    ...
    retailerCalls,
    matched: 0,
    blocked: cartiva500LiveCases().length,
    failed: 0,
    cases: [],
    reason,
  };
...
export async function runCartiva500KrogerLive(): Promise<Cartiva500LiveReport> {
  if (process.env.CARTIVA500_LIVE_KROGER !== "1") {
    return blockedReport("Live Kroger validation is opt-in; set CARTIVA500_LIVE_KROGER=1.");
  }
  ...
  try {
    let locationId = process.env.CARTIVA500_KROGER_LOCATION_ID?.trim();
    if (!locationId) {
      const nearby = await pacedCall(() => findKrogerLocations(
        process.env.CARTIVA500_KROGER_ZIP?.trim() || "75201",
      ));
  ...
  const results: Cartiva500LiveCaseResult[] = [];
  const liveCases = cartiva500LiveCases();
    ...
    try {
      ...
      if (!metadata) {
        ...
        const alreadyValid = independentCandidates.some((candidate) => (
          cartiva500LiveIndependentCandidateReady(
            testCase,
            intent,
            candidate,
            location.locationId,
      ...
      const independentFailure = cartiva500LiveOracleFailure(testCase.id, latest.recommended!);
      const fulfillmentIssue = cartiva500LiveFulfillmentFailure(testCase, latest.recommended!, latest.fulfillment!);
      const disposition = cartiva500LiveCaseDisposition({
        result: latest,
...
export function formatCartiva500LiveReport(report: Cartiva500LiveReport) {
  const lines = [
    "",
    "CARTIVA 500 · LIVE KROGER SUBSET",

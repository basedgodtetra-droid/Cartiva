import catalogSource from "@/tests/fixtures/cartiva-500-catalog.json";
import fixtureSource from "@/tests/fixtures/cartiva-500.json";
...
export type Cartiva500FailureCategory =
  | "PARSING"
  | "TYPO"
  | "ITEM_BOUNDARY"
  | "CATEGORY"
...
interface Cartiva500Case {
  id: string;
  level: 1 | 2 | 3 | 4 | 5;
  input: string;
  tags: Cartiva500FailureCategory[];
  liveKroger?: boolean;
  expectedItems: ExpectedItem[];
}

interface Cartiva500Fixture {
  schemaVersion: 1;
  suite: {
    id: "cartiva-500";
    title: string;
...
export interface Cartiva500ItemOutcome {
  score: Cartiva500Score;
  deadEnd: boolean;
  category?: Cartiva500FailureCategory;
  reason: string;
  parseMs: number;
  matchMs: number;
...
export interface Cartiva500CaseOutcome {
  id: string;
  level: Cartiva500Case["level"];
  input: string;
  score: Cartiva500Score;
  deadEnd: boolean;
  unsafeSelection: boolean;
  category?: Cartiva500FailureCategory;
  reason: string;
  itemOutcomes: Cartiva500ItemOutcome[];
}

export interface Cartiva500Report {
  suiteId: string;
  scoringPolicy: typeof CARTIVA_500_SCORING_POLICY;
  total: number;
  scoreCounts: Record<Cartiva500Score, number>;
  score2Or3Percent: number;
  atLeast1Percent: number;
  ...
  failureCategories: Partial<Record<Cartiva500FailureCategory, number>>;
  performance: {
  ...
  cases: Cartiva500CaseOutcome[];
}

const fixture = fixtureSource as Cartiva500Fixture;
...
function canonicalizeJson(value: unknown): JsonValue {
  ...
  throw new TypeError("Cartiva 500 fingerprints require JSON-compatible values.");
}
...
export function cartiva500FixtureFingerprints() {
  ...
  return {
    algorithm: CARTIVA_500_FINGERPRINT_ALGORITHM,
    oracleRevision: CARTIVA_500_ORACLE_REVISION,
...
function primaryFailureCategory(testCase: Cartiva500Case) {
  ...
  return priority.find((category) => testCase.tags.includes(category)) ?? "OTHER";
}
...
function hydrateProduct(source: CatalogProduct): KrogerProduct {
  ...
  return {
    ...
    sourceUrl: `fixture://cartiva-500/${source.id}`,
    brand: source.brand,
...
function validateFixture() {
  if (fixture.schemaVersion !== 1 || fixture.suite.id !== "cartiva-500") {
    throw new Error("Cartiva 500 fixture schema is not supported.");
  }
  ...
  for (const testCase of fixture.cases) {
    if (ids.has(testCase.id)) throw new Error(`Duplicate Cartiva 500 id: ${testCase.id}`);
    ids.add(testCase.id);
  ...
  for (const level of [1, 2, 3, 4, 5] as const) {
    ...
    if (count < required) {
      throw new Error(`Cartiva 500 level ${level} must retain at least ${required} cases; found ${count}.`);
    }
  }
}
...
function outcome(
  score: Cartiva500Score,
  reason: string,
  options: Partial<Omit<Cartiva500ItemOutcome, "score" | "reason">> = {},
): Cartiva500ItemOutcome {
  return {
    score,
    reason,
...
function queryIssue(expected: ExpectedItem, queries: string[]) {
  ...
  if (missing.length) {
    return { category: "CANDIDATE_RETRIEVAL" as const, reason: `Discovery omitted ${missing.join(", ")}.` };
  }
...
async function automaticOutcome(
  ...
  expected: ExpectedItem,
  testCase: Cartiva500Case,
  parseMs: number,
  scoreCap: Cartiva500Score = 3,
) {
  ...
  const variantOutcomes: Cartiva500ItemOutcome[] = [];
  ...
  for (const variant of variants) {
    ...
    if (expected.packageCount !== undefined && latest.fulfillment?.packageCount !== expected.packageCount) {
      variantOutcomes.push(outcome(0, `${variant.name} produced ${latest.fulfillment?.packageCount ?? "no"} packages; expected ${expected.packageCount}.`, {
        ...baseOptions,
        category: expected.packageCount > 1 ? "MULTI_PACKAGE" : "PACKAGE",
        deadEnd: false,
      }));
      continue;
    }
...
async function runCase(testCase: Cartiva500Case): Promise<Cartiva500CaseOutcome> {
...
export async function runCartiva500(): Promise<Cartiva500Report> {
  validateFixture();
  const cases: Cartiva500CaseOutcome[] = [];
  for (const testCase of fixture.cases) cases.push(await runCase(testCase));

  const scoreCounts: Record<Cartiva500Score, number> = { 0: 0, 1: 0, 2: 0, 3: 0 };
  const levels = {
  ...
  } satisfies Cartiva500Report["levels"];
  const failureCategories: Cartiva500Report["failureCategories"] = {};
  let parseTotal = 0;
  let matchTotal = 0;
  ...
  return {
    ...
    scoringPolicy: CARTIVA_500_SCORING_POLICY,
    total,
    scoreCounts,
    score2Or3Percent,
    atLeast1Percent,
...
export function formatCartiva500Report(report: Cartiva500Report) {
  const lines = [
    "",
    "CARTIVA 500",
    "",
    ...
    ...([1, 2, 3, 4, 5] as const).map((level) => (
      `GROUP ${"ABCDE"[level - 1]}: ${report.levels[level].useful} / ${report.levels[level].total} useful · ${report.levels[level].score2Or3} / ${report.levels[level].total} score 2+`
    )),
    "",
    "FAILURE SUMMARY",
...
export function cartiva500FixtureSummary() {
  validateFixture();
  return {
...
export function cartiva500LiveCases() {
  validateFixture();

...
const SODA_PACKS: GroceryClarificationOption[] = [
  { id: "soda-6", label: "6 pack", value: "6 pack" },
  { id: "soda-12", label: "12 pack", value: "12 pack" },
  { id: "soda-24", label: "24 pack", value: "24 pack" },
];
...
const BREAD_SIZES: GroceryClarificationOption[] = [
  { id: "bread-20", label: "20 oz", value: "20 oz" },
  { id: "bread-24", label: "24 oz", value: "24 oz" },
];
...
function clarificationFor(raw: string, category: string | undefined): GroceryClarification | undefined {
  ...
  if (category === "bread" && !WEIGHT_PATTERN.test(lower)) {
    return { id: "bread-size", prompt: "What loaf size?", shortLabel: "Needs a size", options: BREAD_SIZES };
  }

  if (category === "soda") {
    const needsVariant = /\b(?:coke|coca[-\s]?cola|pepsi|soda)\b/.test(lower)
      && !/\b(?:diet|zero|original|regular)\b/.test(lower);
    if (needsVariant) {
      return { id: "soda-variant", prompt: /\bsoda\b/.test(lower) ? "Which soda?" : "Which version?", shortLabel: "Needs a variety", options: sodaVariantOptions(raw) };
    }
    if (!PACK_PATTERN.test(lower) && !VOLUME_PATTERN.test(lower)) {
      return { id: "soda-pack", prompt: "What pack size?", shortLabel: "Needs a pack size", options: SODA_PACKS };
    }
  }
